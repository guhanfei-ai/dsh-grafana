...
- Legacy single-source migration preserves custom credential references.
- Template variables selected as All expand to their configured options or custom All value. Custom All values retain their intended formatting, and explicit variable overrides can recover an otherwise unresolvable saved selection.
- Log-frame summaries select the message column instead of reporting a timestamp as the last log line.
- Dashboard save and clone links resolve correctly on Grafana deployments served from a sub-path.
- HTTP 200 responses must contain valid JSON; dashboard saves require a confirmed success status, matching UID for updates, and a positive version. Status checks validate the health and search response shapes before reporting credential validity.
- Compatibility with DeepSeek Harness `0.1.5` prereleases: the `@deepseek-ai/dsh-tools` peer range is now `^0.1.0-rc.6 || ^0.1.1-rc.0 || ^0.1.2-alpha.0 || ^0.1.3-alpha.0 || ^0.1.5-alpha.0`, and devDependencies develop against `dsh-tools@0.1.5-rc.1` (superseding the `0.1.3-alpha.2` pin recorded under `0.12.0`). Under node-semver's prerelease matching rules a prerelease is only matched by comparators carrying the same `[major, minor, patch]` tuple, so the previous range — whose prerelease comparators covered tuples `[0,1,0]`, `[0,1,1]`, `[0,1,2]`, and `[0,1,3]` only — did not match `0.1.5-rc.1` despite it falling inside `>=0.1.3-alpha.0 <0.2.0`: installing this plugin into a host carrying dsh-tools `0.1.5-rc.1` was blocked with npm ERESOLVE / pnpm strict peer errors. The `@deepseek-ai/dsh` host package promoted `0.1.5-rc.1` to its `latest` dist-tag (with `0.1.5-rc.2` on `next`), so installing the host on a fresh machine now brings in `dsh-tools@0.1.5-rc.1` and hits the block. No host or client code needed changing, and the surfaces this plugin consumes were re-verified against the installed `0.1.5-rc.1` packages: `defineTool` still requires `output` and keeps the `execute(args, exec)` shape, the `tools/pre-execute` waterfall still yields `PreToolDecision`, `exec.signal` is still an `AbortSignal`, the `settings.describe` envelope is unchanged, and the `settings.plugin.item` keyed slot survives the 0.1.5 slot-catalog reshuffle (the 0.1.5 "Web plugin panel API adjustment" — the old conversation Slot moving to a `main` conversation key — does not touch this plugin's keyed slot; session-data V3 migration is host-layer). `@deepseek-ai/dsh-client-runtime` stays in the client-inject list on purpose: upstream has published nothing beyond `0.1.1-rc.2`, hosts up to `0.1.1-rc.2` still need it, and newer loaders skip the absent graph row without error. The full test suite passes against `dsh-tools@0.1.5-rc.1`.

### Security

- Dashboard writes are bound to the source approved by the user. Changing the resolved source while approval is pending rejects the write, and changes to a source URL or credential reference invalidate snapshots taken before the change.
- Non-JSON responses and save-confirmation errors redact credential-shaped text before it reaches tool output.

## [0.13.0] - 2026-09-09

### Added

- `grafana_datasources` now shows each datasource's configured URL, with an explicit `url="(empty)"` marker when it is blank — a misconfigured datasource (empty URL, failing every query with a 502) used to be indistinguishable from healthy ones in the listing, so it could only be discovered by running a query against it and misreading the failure as a tool fault. No active health probing: with dozens of provisioned datasources per-source probing is too costly and plugin types disagree on health endpoints; the empty-URL marker covers the observed failure mode.

### Fixed

- A dashboard whose datasource-type template variable holds a stale uid no longer fails with a bare upstream `404 Data source not found`. When the panel pipeline resolves a datasource reference, it now remembers where the uid came from; if `/api/ds/query` then answers 404 or 502, the failure (thrown for a single query, or on each per-panel fallback row) gains the actual uid, the template variable it was resolved from, and the way out — pass `variables={"<name>":"<valid uid>"}` to override the saved value (`grafana_datasources` lists the uids and their URLs). The hint fires for **both** ways a variable value can be broken: a uid that no longer exists on the source (index lookup misses, request passes the raw uid through) **and** a uid that is still listed but internally broken (empty URL and the like) — the first live-fire retest caught the hint not firing in the second case, because it had been gated on the index miss; variable references are now traced regardless of whether the index lookup hits. A stale uid saved directly on the panel (not via a variable) gets the matching "edit the panel's datasource" wording instead, a 404/502 on a datasource that resolved cleanly from a direct reference gets no extra text, and neither do 400s (query syntax) — only 404/502 with a traceable origin carry the hint.
- The 2 MB response-size limit no longer reads as a tool fault. `readLimitedText`'s rejection now appends the generic way out (narrow the request — filters, aggregation, a shorter range — or ask for a smaller shape), and `grafana_metric` adds its own guidance on top: for a high-cardinality query, cut the series count with aggregation (`count(up)`, `sum by (job) (up)`, `topk`) or a label filter. Range mode is explicitly called out as not helping — its `maxDataPoints` downsamples points per series but does not reduce the series count, so the same large result exceeds the limit either way. Truncating the response instead of failing is not implementable client-side: `/api/ds/query` answers with one JSON document that must be parsed whole, and `/api/ds/query` exposes no server-side series cap to tighten the request.

## [0.12.3] - 2026-09-09

### Fixed

- `grafana_metric` no longer swallows an in-band query error. `/api/ds/query` can answer HTTP 200 with `results.<refId>.error` (datasource disabled, query refused); the tool used to render that as `series=0`, which reads as "no data". The error is now thrown with its upstream reason, with credential-shaped strings (Bearer tokens, `glsa_`/`glc_` tokens, long base64 runs) redacted before the single-line, length-bounded rendering — an in-band echo of the request credential could previously reach the model verbatim, because this HTTP-200 path never passes through the API-failure translation that already redacts error bodies. The panel pipeline's rendering of the same in-band shape (`summarizeFrames` / `summarizeTrendFrames`, used by `grafana_panel_query` and `grafana_trend`) is redacted identically.
- Truncation disclosure now covers the two remaining silent caps: `grafana_search` reports dropped rows past its 50-row cap on a final `budget:` line, and `grafana_get`'s `summary: true` mode does the same for lines past its 150-line cap, both matching the budget-line convention every other truncating tool already follows. `grafana_search`'s disclosure follows Grafana's real paging contract: `limit` is a page size (the server truncates the single-page response at the requested limit), so the previous local `rows.length > 50` check could never see row 51 and never fired. When the first page comes back full, the tool now fetches page 2 to learn the truth — exactly nothing hidden when page 2 is empty, the exact count on a partial page, an `N+ hidden` lower bound when page 2 is itself full, and an explicit "could not be fetched" line when the follow-up request fails, instead of silently implying the list was complete. That degradation covers only the probe's own faults: when `exec.signal` is aborted (user cancellation or the tool-level deadline) the error propagates unchanged, because a caller that asked to stop must not receive a partial result that reads like a complete one.
- Removing a source no longer reports success while its token survives in the credential store. The removal flow wrapped the `credentials.unset` call in an empty `catch`, so a read-only or temporarily unavailable store still produced a "Saved" state with the orphan token left behind — contradicting the documented promise that removal clears the stored token. The list write still happens first and is not rolled back (a leftover token is an unreferenced credential entry, and a future source gets a fresh UUID-derived ref that never reuses it), but a failed cleanup now surfaces an explicit "source removed, clearing its token failed" message with a retry button that re-issues just the unset, and the notice stays until the retry succeeds. Pending cleanups accumulate per token reference — removing two sources whose token cleanups both fail yields two notices with independent retry buttons, and a successful retry clears only its own entry — and the local source list advances from the known write result instead of waiting for the follow-up read, so a failed resync can no longer leave a card on screen for a source that was already deleted (the sync error is still shown, and a successful token retry re-syncs).
