...
import { folderLabel, folderUidOf, oneLine, parseUid } from './util.js'
...
// 目标源站行：多源站后审批人必须看清「写到哪一台」。grafanaSource 为已解析源站
// （{name, baseUrl}）或解析失败标记（{error}）；为 null 时不出这一行。
export function sourceApprovalLine(grafanaSource) {
  if (!grafanaSource) return null
  if (grafanaSource.error) {
    return `⚠️ Target Grafana source could not be resolved (${oneLine(grafanaSource.error, 200)}). The write will be rejected.`
  }
  if (typeof grafanaSource.name === 'string' && grafanaSource.name) {
    const url = typeof grafanaSource.baseUrl === 'string' && grafanaSource.baseUrl ? ` (${oneLine(grafanaSource.baseUrl, 200)})` : ''
    return `Target Grafana source: ${JSON.stringify(grafanaSource.name)}${url}.`
  }
  return null
}
...
// grafanaSource 为解析出的目标源站（或 {error}），用于文案首行标明写到哪一台。
export function approvalReason(args, snapshot, live = null, diffLines = null, grafanaSource = null) {
  ...
  const sourceLine = sourceApprovalLine(grafanaSource)
  if (!uid || !snapshot) {
    return [
...
export function cloneApprovalReason(args, grafanaSource = null) {
  ...
  const line = `Create a new Grafana dashboard by cloning source uid=${sourceUid}${target}${folder}.`
  const sourceLine = sourceApprovalLine(grafanaSource)
