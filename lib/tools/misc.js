...
import { oneLine, textOut } from '../util.js'
...
export function defineGrafanaSearchTool(rt) {
  return defineTool({
    ...
    presentCall: (args) => ({ card: 'generic', title: `Search Grafana dashboards${typeof args?.query === 'string' && args.query.trim() ? `: ${oneLine(args.query.trim(), 60)}` : ''}`, kind: 'search' }),
    async execute(args, exec) {
      ...
      const truncationHint = 'narrow the query or tag to reduce the result set'
      if (rows.length > MAX_SEARCH_ROWS) {
        // 防御：不合契约的超限单页（真实服务端按 limit 截断响应）仍如实披露。
        lines.push(`budget: ${MAX_SEARCH_ROWS} of ${rows.length} dashboard(s) shown; ${rows.length - MAX_SEARCH_ROWS} hidden (${truncationHint})`)
        ...
        if (hidden === null) {
          lines.push(`budget: ${MAX_SEARCH_ROWS} of ${MAX_SEARCH_ROWS}+ dashboard(s) shown; more may be hidden — the follow-up page could not be fetched (${truncationHint})`)
        } else if (hidden >= MAX_SEARCH_ROWS) {
          // 第二页也满页：总数至少两页，继续翻页没有上限，披露为下界。
          lines.push(`budget: ${MAX_SEARCH_ROWS} of ${MAX_SEARCH_ROWS + hidden}+ dashboard(s) shown; ${hidden}+ hidden (${truncationHint})`)
        } else if (hidden > 0) {
          lines.push(`budget: ${MAX_SEARCH_ROWS} of ${MAX_SEARCH_ROWS + hidden} dashboard(s) shown; ${hidden} hidden (${truncationHint})`)
        }
...
export function defineGrafanaStatusTool(rt) {
  return defineTool({
    ...
    timeoutMs: TOOL_TIMEOUT_MS,
    isConcurrencySafe: () => true,
    presentCall: (args) => ({ card: 'generic', title: `Check Grafana status${args?.source ? ` on ${oneLine(String(args.source), 40)}` : ''}`, kind: 'read' }),
    async execute(args, exec) {
      const srt = rt.forSource(rt.resolveSource(args.source))
      // Grafana 的 /api/health 返回 { commit, database, version }，没有 status
      // 字段；database 才是健康状态（ok / failing）。结构不符就是没探到 Grafana，
      // 报 unknown 会把「被网关拦下的登录页」说成健康检查通过。
      const health = await srt.api('/api/health', {}, exec.signal)
      if (!health || typeof health !== 'object' || Array.isArray(health) || typeof health.database !== 'string') {
        throw new Error('Grafana /api/health did not return the expected health payload (a "database" field); the source did not answer like Grafana. Check the base URL and any proxy in front of it.')
      }
      // 凭证校验同理：search 必须返回一个列表，否则不能判定凭证有效。
      const rows = await srt.authenticatedApi('/api/search?type=dash-db&limit=3', {}, exec.signal)
      if (!Array.isArray(rows)) {
        throw new Error('Grafana /api/search did not return a dashboard list, so the credential could not be validated. Check the token and its permissions.')
      }
      return `health=${health.database}; credential=valid; sampleDashboards=${rows.length}`
    },
  })
}
...
export function defineGrafanaSourcesTool(rt) {
  return defineTool({
    ...
    timeoutMs: TOOL_TIMEOUT_MS,
    isConcurrencySafe: () => true,
    presentCall: () => ({ card: 'generic', title: 'List configured Grafana sources', kind: 'read' }),
    async execute() {
