...
import { ADHOC_DATASOURCE_SUPPORT_TEXT, LOKI_DATASOURCE_TYPE, PROMETHEUS_DATASOURCE_TYPE } from '../constants.js'
import {
  applyAdhocFilters,
...
export function defineGrafanaQueryTool(rt) {
  return defineTool({
    ...
    async execute(args, exec) {
      ...
      // 收集所有生效的 adhoc filters（按变量定义顺序），附带 datasource 绑定信息。
      const adhocEntries = []
      ...
      for (const variable of list) {
        ...
        let boundUid = null
        if (typeof ds === 'string') boundUid = ds
        else if (ds && typeof ds === 'object' && typeof ds.uid === 'string') boundUid = ds.uid
        adhocEntries.push({ filters, boundUid })
      }
      ...
      // uid 引用 $datasource 变量）时才请求一次 GET /api/datasources；
      // 权限不足或响应异常时为 null，引用走原 uid 透传（Grafana 可自行解析裸 uid）。
      ...
      const refNeedsResolution = (ref) => {
        ...
        return typeof ref.uid === 'string' && ref.uid.trim().startsWith('$')
      }
      if (selected.some((panel) => panel.targets.some((target) => target && typeof target === 'object' && target.hide !== true && refNeedsResolution(target.datasource ?? panel.datasource)))) {
      ...
      for (const panel of selected) {
        for (const target of panel.targets) {
          ...
          try {
            ...
            skipped.push({ panel, message: `datasource could not be resolved: ${oneLine(error.message, 150)}` })
            continue
          }
      ...
      if (queries.length === 0) {
        // 所有面板被跳过：逐面板列出 id/标题与跳过原因（旧格式数据源引用、
        // 未解析模板变量等），绝不笼统地只说 "no executable query"。
        const detail = skipped
          .map((entry) => `panel id=${entry.panel.id} ${JSON.stringify(oneLine(entry.panel.title ?? '', 40))}: ${oneLine(entry.message, 120)}`)
          .join('; ')
