...
import { isValidTimeInput, mergeVariableOverrides, parseDashboardUrl, summarizeFrames } from '../query.js'
import { oneLine, textOut } from '../util.js'
import { SOURCE_PARAM } from '../constants.js'
...
export function defineGrafanaPanelQueryTool(rt) {
  return defineTool({
    ...
    presentCall: (args) => ({ card: 'generic', title: `Query dashboard panels ${oneLine(String(args?.urlOrUid ?? ''), 60)}`, kind: 'read' }),
...
// 旧名转发 stub：0.12.0 把 grafana_query 改名为 grafana_panel_query。存量会话与自定义
...
export function defineGrafanaQueryAliasTool() {
  return defineTool({
    name: 'grafana_query',
    description: 'Deprecated name, do not call: grafana_query was renamed to grafana_panel_query in 0.12.0 (same parameters, same output). This stub exists only so existing sessions learn the new name; every call fails with a pointer to it.',
    parameters: {},
    output: { schema: { type: 'string' }, render: (_args, value) => textOut(value) },
    async execute() {
      throw new Error('grafana_query was renamed to grafana_panel_query in 0.12.0. The parameters and output are unchanged — call grafana_panel_query with the same arguments.')
    },
  })
}
