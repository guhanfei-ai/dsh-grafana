...
// 只取 to 作为求值时刻，range 校验正的时间跨度与 90 天上限。
...
export function defineGrafanaCompareTool(rt) {
  return defineTool({
    ...
    parameters: {
      ...
      from: { type: 'string', description: 'Optional range start: relative like now-15m or a 13-digit epoch millisecond timestamp. Defaults to now-15m. Ignored in instant mode, which evaluates at the to instant. In range mode it must be strictly earlier than to (a zero-length range has no sampling step) and the span may not exceed 90 days.' },
