...
async function runOneSource({ rt, sourceName, datasource, query, mode, fromMs, toMs, points, signal }) {
  ...
  try {
    ...
    for (const frame of frames) {
      ...
      if (series.length >= MAX_COMPARE_SERIES_PER_SOURCE) break
    }
    return {
      name: sourceName,
      ok: true,
...
export function defineGrafanaCompareTool(rt) {
  return defineTool({
    ...
    presentCall: (args) => ({ card: 'generic', title: `Compare metric ${oneLine(String(args?.expr ?? ''), 60)}`, kind: 'read' }),
    async execute(args, exec) {
      ...
      // 已经落地、另一半才报「unknown source」。
      const resolvedSources = unique.map((name) => {
        try { return rt.resolveSource(name) }
      ...
      const results = await Promise.all(unique.map((name, index) => runOneSource({
        rt,
        sourceName: name,
        datasource,
        query,
