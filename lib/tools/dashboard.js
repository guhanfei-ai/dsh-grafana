...
export function defineGrafanaGetTool(rt) {
  return defineTool({
    ...
    // 不声明并行：全量读取会记录写快照（共享写状态），保持独占调度。
    presentCall: (args) => ({ card: 'generic', title: `Get dashboard ${oneLine(String(args?.urlOrUid ?? ''), 60)}`, kind: 'read' }),
...
export function defineGrafanaPushTool(rt) {
  return defineTool({
    ...
    output: { schema: { type: 'string' }, render: (_args, value) => textOut(value) },
    timeoutMs: TOOL_TIMEOUT_MS,
    // 写入工具：不声明并行（走审批门、改共享状态），但保留原生调用卡片。
    presentCall: (args) => ({ card: 'generic', title: `Push dashboard changes (${oneLine(String(args?.changeSummary ?? ''), 60)})`, kind: 'edit' }),
    async execute(args, exec) {
      ...
      const resolvedUrl = resolveDashboardUrl(baseUrl, result.url)
      const dashboardUrl = resolvedUrl === null ? 'omitted' : resolvedUrl
      const urlNote = resolvedUrl === null ? '\nWarning: Grafana returned a dashboard URL outside this source (cross-origin, embedded credentials, or a non-HTTP scheme); the URL was omitted.' : ''
      return `Dashboard updated: uid=${result.uid} status=${result.status} version=${result.version} url=${dashboardUrl}${urlNote}\nChanges: ${changeSummary}\nFetch the dashboard again before making another write.`
    },
  })
}
...
export function defineGrafanaCloneTool(rt) {
  return defineTool({
    ...
    timeoutMs: TOOL_TIMEOUT_MS,
    // 写入工具：不声明并行（走审批门、改共享状态），但保留原生调用卡片。
    presentCall: (args) => ({ card: 'generic', title: `Clone dashboard ${oneLine(String(args?.sourceUrlOrUid ?? ''), 60)}`, kind: 'edit' }),
    async execute(args, exec) {
