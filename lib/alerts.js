...
// active，而模型要问的是「现在还有没有人在被叫」，两者不一致时以前者为准。
// 上游状态枚举是 Alertmanager v2 的 active/suppressed/unprocessed（Grafana 的
// GettableAlerts 直接复用该类型），但「正在烧」在工具对外的一侧叫 firing：
// 只认 firing 会把真实的 active 告警归成 unknown 并在默认查询里整批漏报。
...
export function alertState(alert) {
  ...
  if (raw === 'active' || raw === 'firing') return 'firing'
  if (raw === 'suppressed' || raw === 'unprocessed') return raw
  return 'unknown'
}
...
// 规则与大盘的关联：Grafana 标准放在 annotations.__dashboardUid__（或旧名
// dashboardUid）；data[].model 里也可能带 dashboardUid / __dashboardUid__。
// 逐个结构化字段精确匹配，不再对 JSON.stringify 做 uid 子串推断——查询文本
// 中偶然出现的 UID 子串会误匹配到不相关的大盘。
function ruleReferencesDashboard(rule, uid) {
  const annotations = bag(rule?.annotations)
  for (const key of DASHBOARD_UID_KEYS) {
    if (uidOr(annotations[key]) === uid) return true
  }
  for (const step of Array.isArray(rule?.data) ? rule.data : []) {
    const model = bag(step?.model)
    for (const key of DASHBOARD_UID_KEYS) {
      if (uidOr(model[key]) === uid) return true
    }
  }
  return false
}
...
// ── 规则评估状态（/api/prometheus/grafana/api/v1/rules）─────────────────────
// 与 Alertmanager 实例视角互补：AM 回答「现在有什么在烧」，这里回答「规则自己
// 处在哪个评估阶段」（inactive/pending/firing；recording 规则没有告警语义，
// 标 recording；上游将来新增的状态枚举一律归 unknown 如实报出）。
const RULE_EVAL_STATES = new Set(['inactive', 'pending', 'firing'])
export const RULE_STATE_FILTERS = new Set(['inactive', 'pending', 'firing', 'recording', 'unknown', 'all'])

// 把分组载荷拍平成行：groups[].file 是文件夹名（Grafana 在该接口用 file 承载
// 文件夹），groups[].name 是评估组名。解析处处容错，畸形条目降级为占位值。
export function collectRuleStates(payload) {
  ...
  for (const group of groups) {
    ...
    for (const rule of Array.isArray(g.rules) ? g.rules : []) {
      ...
      rows.push({
        name: String(r.name ?? ''),
        state: RULE_EVAL_STATES.has(rawState) ? rawState : (String(r.type ?? '') === 'recording' ? 'recording' : 'unknown'),
        folder: String(g.file ?? ''),
        group: String(g.name ?? ''),
        alerts: Array.isArray(r.alerts) ? r.alerts.length : 0,
        labels: bag(r.labels),
        annotations: bag(r.annotations),
      })
    }
  }
  return rows
...
export function filterRuleStates(list, { state, folderContains, labelContains, dashboardUid } = {}) {
  const wanted = String(state ?? '').trim().toLowerCase() || 'all'
  const folder = String(folderContains ?? '').trim().toLowerCase()
  const needle = String(labelContains ?? '').trim().toLowerCase()
  const uid = String(dashboardUid ?? '').trim()
  const rows = []
  for (const row of Array.isArray(list) ? list : []) {
    ...
    if (wanted !== 'all' && row.state !== wanted) continue
    if (folder && !String(row.folder ?? '').toLowerCase().includes(folder)) continue
    // 匹配面比活跃告警多一个规则名：状态接口的载荷里名字是主标识。
    if (needle && !`${String(row.name ?? '')} ${searchText(row)}`.toLowerCase().includes(needle)) continue
    if (uid) {
      const annotations = bag(row.annotations)
      if (!DASHBOARD_UID_KEYS.some((key) => uidOr(annotations[key]) === uid)) continue
    }
    rows.push(row)
  }
  return rows
...
export function formatRuleStateRows(list) {
  return (Array.isArray(list) ? list : []).map((row) => [
    `rule-state ${JSON.stringify(oneLine(row?.name ?? '', 120) || '(unnamed rule)')}`,
    `state=${oneLine(row?.state ?? '', 20) || 'unknown'}`,
    `folder=${JSON.stringify(oneLine(row?.folder ?? '', 60) || '?')}`,
    `group=${JSON.stringify(oneLine(row?.group ?? '', 60) || '?')}`,
    `alerts=${Number.isFinite(row?.alerts) ? row.alerts : 0}`,
  ].join(' '))
}
