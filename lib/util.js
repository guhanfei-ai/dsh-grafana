...
// 把 Grafana 返回的大盘相对路径拼成可打开的完整地址。
// 服务端返回的路径已经包含 AppSubUrl（Grafana v11 的 GetDashboardURL 会把它
// 加进去），因此不能直接拼在 baseUrl 后面：子路径部署会得到
// /grafana/grafana/d/<uid>/... 这类坏链接。按根相对路径语义解析——已带
// baseUrl 前缀就只补 origin，否则补上 baseUrl 的 pathname。
//
// 输入是不可信的上游数据，只承认两种形态：根相对路径，以及与配置源站同 origin
// 且不含 userinfo 的绝对 HTTP(S) URL。其余一律返回 null（调用方省略该行）：
// javascript:/data:/ftp: 这类 scheme 被宿主链接化就是可点击的攻击面，而
// https://user:pass@host/ 会把凭证回显给用户、https://host@evil/ 的 origin 其实
// 是 evil——URL 的 origin 看不见 userinfo，必须单独检查。
export function resolveDashboardUrl(baseUrl, url) {
  const path = String(url ?? '').trim()
  if (!path) return '?'
  if (/^https?:\/\//i.test(path)) {
    // 绝对 URL：必须与配置 baseUrl 同源（scheme + host + port）才可信，否则视为
    // 不可信链接省略。直接返回恶意绝对 URL 会让用户误以为是自己 Grafana 的可信
    // 链接（trusted-link confusion）；比较 origin 而非 startsWith(baseUrl)，可挡住
    // grafana.example.com.evil.com 这类前缀伪装与自定义端口。
    try {
      const given = new URL(path)
      const base = new URL(String(baseUrl ?? '').trim())
      if (given.username || given.password) return null
      if (given.origin === base.origin) return path
      return null
    } catch {
      return null
    }
  ...
  try {
    ...
    if (root && (path === root || path.startsWith(`${root}/`))) return `${base.origin}${path}`
    return `${base.origin}${root}${path}`
  } catch {
    // baseUrl 非法（配置未就绪）：退回朴素拼接，至少不丢信息。
...
// 响应超限的通用建议句：上限是字节级的硬顶（响应须整体解析，无法按条目截断），
// 用户能做的只有把请求收窄或换更小的形状（大盘的 summary 模式、查询的聚合）。
const RESPONSE_LIMIT_ADVICE = ' Narrow the request (filters, aggregation, a shorter range) or ask for a smaller shape.'
...
export async function readLimitedText(response, maxBytes = MAX_RESPONSE_BYTES) {
  ...
  if (Number.isFinite(contentLength) && contentLength > maxBytes) {
    ...
    throw new Error(`Grafana response is too large (${contentLength} bytes; limit ${maxBytes} bytes).${RESPONSE_LIMIT_ADVICE}`)
  }

  if (!response.body?.getReader) {
    const text = await response.text()
    if (byteLength(text) > maxBytes) throw new Error(`Grafana response exceeds the ${maxBytes}-byte limit.${RESPONSE_LIMIT_ADVICE}`)
    return text
  }
  ...
  while (true) {
    ...
    if (total > maxBytes) {
      await reader.cancel()
      throw new Error(`Grafana response exceeds the ${maxBytes}-byte limit.${RESPONSE_LIMIT_ADVICE}`)
    }
    chunks.push(value)
  }
