...
// grafana_search 的行数上限（与 /api/search 的 limit 同值；超出走预算行披露）。
export const MAX_SEARCH_ROWS = 50
// 工具级预算必须覆盖「单次 api() 调用的最坏耗时」：GET 含一次重试为
// 2×REQUEST_TIMEOUT_MS + 200ms 重试间隔 ≈ 30.2s。status/push/clone 各有两个
// 顺序请求（status 双 GET 最坏 60.4s；push/clone 预写 GET 30.2s + POST 15s），
// 故工具级取 75s，留出凭证解析与头部处理的余量，保证请求自身的超时文案
// 先于宿主的工具级 abort 出现（模型才能把根因归到正确的请求上）。
