...
// 各工具统一的 source 参数说明（单处措辞，全部 tools 共用，保持一致）。
export const SOURCE_PARAM = { type: 'string', description: 'Optional Grafana source name (call grafana_sources to list configured sources); omit to use the default source.' }
// grafana_search 的行数上限（与 /api/search 的 limit 同值）。limit 是页大小而非
// 总量上限（服务端会把单页响应截到 limit），故满页时需再取 page=2 探测真实丢弃
// 数量并走预算行披露；探测细节见 lib/tools/misc.js。
export const MAX_SEARCH_ROWS = 50
// 工具级预算必须覆盖「单次 api() 调用的最坏耗时」：GET 含一次重试为
// 2×REQUEST_TIMEOUT_MS + 200ms 重试间隔 ≈ 30.2s。status/search/push/clone 各有
// 两个顺序请求（status 双 GET 最坏 60.4s；search 第一页 + 满页探测第二页同为
// 双 GET 60.4s；push/clone 预写 GET 30.2s + POST 15s），故工具级取 75s，留出
// 凭证解析与头部处理的余量，保证请求自身的超时文案先于宿主的工具级 abort 出现
// （模型才能把根因归到正确的请求上）。
export const TOOL_TIMEOUT_MS = 75_000
...
// Grafana 服务端表达式引擎的伪数据源：它按 refId 引用同一次请求里的其它查询，
// 由服务端求值（不接受采样键，引用也不是模板变量）。
export const EXPRESSION_DATASOURCE_TYPE = '__expr__'
// 模板变量全选（includeAll）的哨兵值：Grafana 保存 All 时写进 current.value，
// 查询前必须按 allValue 或 options 展开，不能把它当成真正的筛选值。
...
// 本地分页（page/limit + 披露行，见 lib/budget.js 的 paginate）。
export const MAX_DATASOURCE_ROWS = 40
// 本地分页的页码上限：页码是「继续查看」的游标，不设防会让模型拿巨大页码
// 空转；10_000 页 × 最小页大小已远超任何真实部署的规模。
...
// 超时：裸查询单请求与既有 QUERY_REQUEST_TIMEOUT_MS 同档。工具级预算按
// 「各阶段最坏耗时之和 + 余量」推导：metric 是 datasources GET（含重试 30.2s）
// + 查询 POST（30s）= 60.2s → 75s；alerts 是两次顺序 GET（各含重试 30.2s）
// = 60.4s → 75s；trend 的逐面板降级没有总预算上限（面板数可达 50），超限时由
// runPanelQueries 的 abort 检查把未重试面板显式记为预算耗尽，工具级维持 120s。
export const METRIC_REQUEST_TIMEOUT_MS = 30_000
export const METRIC_TOOL_TIMEOUT_MS = 75_000
export const TREND_TOOL_TIMEOUT_MS = 120_000
export const ALERT_TOOL_TIMEOUT_MS = 75_000

// grafana_compare：跨多源站执行同一查询并横向比较的安全边界。
// 上限 10：与 MAX_SOURCES=50 留出余量，但仍可容纳同一团队的多个 region/环境；
// 同时保证并发下整体墙钟时间可控（最坏单源 chain 60.2s × 1 headroom ≈ 75s）。
export const MAX_COMPARE_SOURCES = 10
// 至少 2 个：单源比较无意义；少于 2 应当在工具层就拒绝，不发请求。
export const MIN_COMPARE_SOURCES = 2
// 每源站最多展示的 series 数：跨源比较关心的是「每个环境的整体值」，通常聚合
// 查询（sum / avg / histogram_quantile）只返回 1 条 series；偶尔查询本身有高
// 基数时截断既防止输出爆炸也提示用户加聚合。
export const MAX_COMPARE_SERIES_PER_SOURCE = 5
// grafana_compare 工具级超时：N 个 source 并发时总墙钟时间由「最长那条链」决定，
// 不是 sources 数 × 单链长。每个 source 的链最坏：
//   /api/datasources (GET 含一次重试 ≈ 30.2s) + /api/ds/query (POST 30s) ≈ 60.2s。
// 并发下最长链仍约 60s，再加 ~30s 余量（凭证解析 + 索引差异 + 渲染）→ 90s。
// 与 QUERY_TOOL_TIMEOUT_MS=90_000 同档，且不会比单源 metric 慢太多。
export const COMPARE_TOOL_TIMEOUT_MS = 90_000
