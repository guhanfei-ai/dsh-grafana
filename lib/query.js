...
export function normalizeAdhocFilters(input, variableName) {
  ...
  for (let i = 0; i < input.length; i++) {
    ...
    if (item.condition !== undefined && item.condition !== 'AND') {
      throw new Error(`adhoc filters for variable "${variableName}" contain condition ${JSON.stringify(item.condition)} at index ${i}, which grafana_query cannot translate (only AND is supported). Override the variable with AND filters or clear it with [].`)
    }
