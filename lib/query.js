...
export function bucketizeSeries(values, buckets, timeValues, fromMs, toMs) {
  ...
  if (Array.isArray(timeValues) && Number.isFinite(fromMs) && Number.isFinite(toMs) && toMs > fromMs) {
    ...
    for (let i = 0; i < rawNumbers.length && i < timeValues.length; i += 1) {
      ...
      const clamped = Math.max(fromMs, Math.min(toMs - 1, time))
      const bucketIndex = Math.min(size - 1, Math.floor((clamped - fromMs) / bucketMs))
      sums[bucketIndex] += value
      counts[bucketIndex] += 1
      pairCount += 1
    }
    if (pairCount === 0) return numbers.slice()
    const out = []
        ...
        for (const { name, values: colValues } of numericColumns) {
          ...
          if (!stats) {
            ...
            if (timeIndex < 0) {
              ...
              lines.push(`${line} points=${colValues.length} buckets=${buckets.length} range=${oneLine(range ?? '?', 60) || '?'} ${detail} trend=${classifyTrend(buckets, stats)} spark=${renderSparkline(buckets)}`)
            }
          }
          seriesLineIndexes.push(lines.length - 1)
        }
...
export function summarizeMetricResult(frames, { mode, datasource, expr, range, points, total, fromMs, toMs, stepMs } = {}) {
  ...
  for (const frame of list) {
    ...
    for (const { name, values: colValues } of numericColumns) {
      ...
      const buckets = bucketizeSeries(colValues, points, timeValues, fromMs, toMs)
      lines.push(`series ${label} points=${colValues.length} buckets=${buckets.length} ${statsFragment(colValues, stats)} trend=${classifyTrend(buckets, stats)} spark=${renderSparkline(buckets)}`)
    }
  }
  return lines
}
...
export function summarizeCompareSeries(frame, { mode, points, fromMs, toMs }) {
  ...
  for (const { name, values: colValues } of numericColumns) {
    ...
    out.push({
      ...
      first: firstFinite(colValues),
      last: stats.last,
      min: stats.min,
      max: stats.max,
      avg: stats.avg,
      trend: classifyTrend(buckets, stats),
...
export function summarizeCompareResult(perSourceResults, options) {
  ...
  const allSuccessfulAreSingles = list.length > 0 && list.every((r) => !r.ok || r.noData || (Array.isArray(r.series) && r.series.length === 1 && isNumericSeries(r.series[0])))

  if (allSuccessfulAreSingles && successfulSingles.length >= 1) {
    // 分支 ①：紧凑表 + 数学 summary
