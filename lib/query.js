...
export function summarizeFrames(records, results, budget) {
  ...
  for (const record of records) {
    ...
    for (const frame of shown) {
      lines.push(`  query ${record.originalRefId}: ${summarizeFrame(frame)}`)
      seriesLineIndexes.push(lines.length - 1)
    }
    seriesShown += shown.length
...
export function bucketizeSeries(values, buckets) {
  const numbers = (Array.isArray(values) ? values : []).filter((value) => typeof value === 'number' && Number.isFinite(value))
  if (numbers.length === 0) return []
  const size = Math.max(1, Math.floor(Number(buckets)))
  if (!Number.isFinite(size) || numbers.length <= size) return numbers.slice()
...
// 帧列定位：与 summarizeFrame 同一套规则（首个 number 列 / 首个 time 列）。
function frameColumns(frame) {
  const fields = Array.isArray(frame?.schema?.fields) ? frame.schema.fields : []
  const values = Array.isArray(frame?.data?.values) ? frame.data.values : []
  const numberIndex = fields.findIndex((field) => field?.type === 'number')
  const timeIndex = fields.findIndex((field) => field?.type === 'time')
  const numeric = numberIndex >= 0 && Array.isArray(values[numberIndex]) ? values[numberIndex] : null
  // 非数值帧（Loki 日志行等）取正文列而不是首列：首列常常是时间戳。
  const textIndex = logTextColumnIndex(fields, values, timeIndex)
  const fallback = textIndex >= 0 ? values[textIndex] : values.find(Array.isArray)
  return { numeric, column: numeric ?? (Array.isArray(fallback) ? fallback : []), timeIndex }
}
...
export function summarizeTrendFrames(records, results, { points, budget, range } = {}) {
  const lines = []
  const seriesLineIndexes = []
  ...
  for (const record of Array.isArray(records) ? records : []) {
    ...
    for (const frame of frames) {
      const { numeric, column, timeIndex } = frameColumns(frame)
      // 点数总预算按 series 整条计：超出就整条丢弃，而不半截切（半截序列会渲染出假走向）。
      if (column.length > pointBudget) {
        dropped += 1
        continue
      }
      pointBudget -= column.length
      rendered += 1
      const line = `${head} ${selectorLabel(frame)}`
      if (!numeric) {
        // 非数值帧（Loki 日志行等）：报行数与末值，不编造走向。
        const last = column.length > 0 ? oneLine(column[column.length - 1], 120) : ''
        lines.push(`${line} lines=${column.length}${last ? ` last=${JSON.stringify(last)}` : ''}`)
      } else {
        const stats = numericStats(column)
        if (!stats) {
          lines.push(`${line} rows=${column.length} (no numeric values)`)
        } else {
          const detail = statsFragment(column, stats)
          if (timeIndex < 0) {
            // 表格帧（terms 聚合等）：没有时间轴，走向无从谈起。
            lines.push(`${line} rows=${column.length} ${detail} trend=${TABLE_FRAME_TREND}`)
          } else {
            const buckets = bucketizeSeries(column, points)
            lines.push(`${line} buckets=${buckets.length} range=${oneLine(range ?? '?', 60) || '?'} ${detail} trend=${classifyTrend(buckets, stats)} spark=${renderSparkline(buckets)}`)
          }
        }
      }
      seriesLineIndexes.push(lines.length - 1)
...
export function summarizeMetricResult(frames, { mode, datasource, expr, range, points, total } = {}) {
  ...
  for (const frame of list) {
    const { numeric, column, timeIndex } = frameColumns(frame)
    const label = selectorLabel(frame)
    if (!numeric) {
      const last = column.length > 0 ? oneLine(column[column.length - 1], 120) : ''
      lines.push(`series ${label} lines=${column.length}${last ? ` last=${JSON.stringify(last)}` : ''}`)
      continue
    }
    const stats = numericStats(column)
    if (!stats) {
      lines.push(`series ${label} rows=${column.length} (no numeric values)`)
      continue
    }
    if (mode !== 'range' || timeIndex < 0) {
      // instant 是单点求值；range 下的表格帧（无时间轴）也走这一支：多点时报
      // min/max/avg，避免把「最后一行的值」误当成整个结果。
      const detail = column.length > 1
      ...
      lines.push(`series ${label} ${detail}${column.length > 1 ? ` rows=${column.length}` : ''}`)
      continue
    }
    const buckets = bucketizeSeries(column, points)
    lines.push(`series ${label} buckets=${buckets.length} ${statsFragment(column, stats)} trend=${classifyTrend(buckets, stats)} spark=${renderSparkline(buckets)}`)
  }
  return lines
}
