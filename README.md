...
Click **Add source** to create one, **Remove source** to delete it (its stored token is cleared too — if that cleanup fails, the card says so and offers a retry instead of reporting success), and **Set as default** to choose which source is used when a tool call omits `source`. **Save** persists the whole list; every name and URL is validated before anything is written, so an invalid entry never leaves a half-saved state.
