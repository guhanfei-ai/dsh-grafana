...
`npm run verify` → 76/76。含 `grafana_query` 历轮新增（legacy 字符串 uid 索引解析、`$datasource` 解析与 “default” 映射、索引 403 透传、透传+adhoc 报错、promql/loki 裸多值 `(a|b)` 渲染含带点号值、ES 正则成功路径与空模式报错），以及本轮多源站新增：
...
| 4 | 按名称选源 | 对话中对某工具传 `source: "<名称>"` | 请求发往该源站 URL（可用 `grafana_health` 验证） | ⬜ |
